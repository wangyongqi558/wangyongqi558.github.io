...
* **Settings**: Training on **Base** categories and testing on **Novel** (unseen predicates) and **All** (standard) splits.

### 3.2 Comparison with State-of-the-Art (Main Results)
...
**Table 1: Performance on the VidOR Dataset (Ground-truth Trajectories)**
| Split | Method | mAP (%) | R@50 | R@100 |
| :--- | :--- | :--- | :--- | :--- |
| **Novel** | CLIP | 1.08 | 5.48 | 7.20 |
| | RePro | - | 7.20 | 8.35 |
| | **Ours** | **3.58** | **9.22** | **11.53** |
| **All** | VidVRD-II | - | 24.81 | 34.11 |
| | RePro | - | 27.11 | 35.76 |
| | **Ours** | **38.52** | **33.44** | **43.80** |



### 3.3 Ablation Studies
We conducted extensive ablations on VidVRD to verify each component:
* **Multi-modal Synergy**: Combining both visual and language prompting yields the best performance (25.57% AVG mAP) compared to using either alone.
* **Spatio-temporal Modeling**: Both spatial and temporal modules are essential; removing spatial modeling leads to a significant performance drop.
* **Prompt Design**: Our vision-guided language prompting (Continuous + Conditional) outperforms manual templates by a wide margin, especially in the Novel split.
